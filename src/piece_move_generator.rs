...
mod tests {
    ...
    #[test]
    fn test_is_move_legal() {
        let fen = "k1K5/8/8/8/8/1p6/2P5/N7 w - - 0 1";
        let mut game = Game::from_fen(fen).unwrap();

        assert!(game.is_move_legal(PieceMove::new(C2, C3, QUIET_MOVE)));
        assert!(game.is_move_legal(PieceMove::new(C2, B3, CAPTURE)));
        assert!(game.is_move_legal(PieceMove::new(A1, B3, CAPTURE)));
        assert!(game.is_move_legal(PieceMove::new(C2, C4, DOUBLE_PAWN_PUSH)));
        assert!(game.is_move_legal(PieceMove::new(C8, C7, QUIET_MOVE)));
        assert!(game.is_move_legal(PieceMove::new(C8, D7, QUIET_MOVE)));
        assert!(game.is_move_legal(PieceMove::new(C8, D8, QUIET_MOVE)));

        assert!(!game.is_move_legal(PieceMove::new_null()));

        assert!(!game.is_move_legal(PieceMove::new(H1, H5, QUIET_MOVE)));

        // Cannot be done with pseudo legal move checking
        //assert!(!game.is_move_legal(PieceMove::new(C8, B8, QUIET_MOVE))); // Illegal
        //assert!(!game.is_move_legal(PieceMove::new(C8, B7, QUIET_MOVE))); // Illegal
    }
